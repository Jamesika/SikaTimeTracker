...
- GitHub 风格年度热力图，采用七档由浅至深的绿色与一档金黄色配色，每档一小时，超过八小时沿用最高档颜色；支持分类筛选、时间汇总和可交互的图形时间轴；
- 时间轴按每小时分隔，支持以鼠标位置为中心滚轮缩放；自动裁掉首条活动前与末条活动后的空白，并以最少行数分开展示重叠区段；
- 图形时间轴下方按当天累计时长展示软件使用排行，并以水平柱状图直观比较；
- 重叠活动在时间轴中分行展示；每日、每周及时间轴累计按时间区间去重，同一软件内部的重叠记录也只计一次。
- 同一软件、分类和网站的相邻活动按设置的间隔合并展示（默认 60 秒，0 为关闭），窗口标题变化不会打断连续使用；短暂间隙计入时间轴、软件排行、每日和每周累计。原始记录保留，其他软件或分类的记录会阻止跨段合并。
...
- 空闲阈值默认 5 分钟，最短记录时长默认 5 秒；历史活动先合并连续使用，再按当前设置隐藏过短的活动。升级保留已经保存的自定义设置。
