...
public sealed class ApplicationSettingsService
{
    ...
    public async Task<AppPreferences> LoadAsync(CancellationToken cancellationToken = default)
    {
        return new AppPreferences
        {
            ...
            IdleThresholdMinutes = await GetIntAsync("IdleThresholdMinutes", AppPreferences.DefaultIdleThresholdMinutes, 1, 120, cancellationToken),
            MinimumActivitySeconds = await GetIntAsync("MinimumActivitySeconds", AppPreferences.DefaultMinimumActivitySeconds, 0, 60, cancellationToken),
            MergeGapSeconds = await GetIntAsync("MergeGapSeconds", AppPreferences.DefaultMergeGapSeconds, 0, 300, cancellationToken),
